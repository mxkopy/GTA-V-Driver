...
public class IPCFlags
{
    public const int N_FLAGS = 8;
    ...
    public IPCFlags(int n = N_FLAGS, string tag = FLAGS_TAG)
    {
        int size = -(n / -8);
...
public class IPCChannel
{
    ...
    public IPCChannel(int size, string tag)
    {
        this.size = size;
...
public class IPCMappable: Attribute
{
    ...
    public IPCMappable(int position, int count=-1)
    {
        this.position = position;
    }
}

public class IPCMap<T> where T: class, new()
{
    public T map;
    ...
    {
        return Attribute.GetCustomAttribute(field, typeof(IPCMappable)) != null && typeof(Array).IsAssignableFrom(field.FieldType);
    }
    private static IPCMappable GetAttribute(FieldInfo field)
    {
        return (IPCMappable) Attribute.GetCustomAttribute(field, typeof(IPCMappable));
    }
...
public class MappedIPCChannel<T>: IPCChannel where T: class, new()
{

    public IPCMap<T> map = new IPCMap<T>();
    ...
    public MappedIPCChannel(string tag): base(IPCMap<T>.Length, tag)
    {
    }
}
public class GameState
{
    [IPCMappable(position: 0)]
    public float[] CAM = new float[3];

    [IPCMappable(position: 1)]
    public float[] VEL = new float[3];

    [IPCMappable(position: 2)]
    public int[] DMG = new int[1];
}

class GameIPC: IPCFlags
{

    public MappedIPCChannel<GameState> GameStateChannel = new MappedIPCChannel<GameState>("game_state.ipc");
